...
const STRINGS = {
  ...
  btn_record: { zh: "🏅 战绩", en: "🏅 Record" },
  record_title: { zh: "战绩", en: "Competitive record" },
  record_unavailable: { zh: "战绩暂时拉取不到——稍后再试。", en: "The record is unavailable right now — try again shortly." },
  record_empty: { zh: "还没有完成的对局。", en: "No completed matches yet." },
  record_rating_line: { zh: "<b>{game}</b> {rating} 分 · {wins}胜{losses}负 · 胜率 {rate}%", en: "<b>{game}</b> {rating} · {wins}W {losses}L · {rate}% win rate" },
  record_recent_title: { zh: "近 5 场(点开看回放):", en: "Last 5 matches (tap for the replay):" },
  record_achievements: { zh: "🏆 成就 {count} 项——详情见 Agent 主页。", en: "🏆 {count} achievements — details on the agent page." },
  cmd_record: { zh: "战绩与回放", en: "Record & replays" },
