...
export async function installBridgeService(
  ...
  try {
    ...
    // R13-F04: a system unit runs the Agent as root. Do not do so silently —
    // surface a warning recommending a dedicated non-root user. Automatic update
    // is off by default so this root service does not self-run `npm install -g`.
    return {
      ...target,
      linger: "not_needed",
      warning:
        `Installed ${AIFIGHT_SERVICE_NAME} as a SYSTEM service — it runs as root. ` +
        "For least privilege, run it as a dedicated non-root user instead " +
...
function renderSystemdUnit(target: BridgeServiceTarget): string {
  const isSystem = target.platform === "linux-systemd-system";
  const wantedBy = isSystem ? "multi-user.target" : "default.target";
  return [
    "# Auto-generated by AIFight. Re-run `aifight service install` to refresh.",
    ...(isSystem
      ? [
          "#",
          "# R13-F04: this is a SYSTEM unit and will run the Agent as root. AIFight",
          "# recommends running it as a dedicated non-root user instead: create one,",
          "# give it its own AIFIGHT_RUNTIME_HOME, and uncomment the `User=`/`Group=`",
          "# lines below (root is only needed to write this unit file, not to run the",
          "# Agent). Automatic self-update is OFF by default (set AIFIGHT_AUTO_UPDATE=1",
          "# to enable) precisely so a system service does not silently run",
          "# `npm install -g` as root.",
        ]
      : []),
    "",
    "[Unit]",
    ...
    ...(isSystem
      ? [
          "# Recommended: run as a dedicated non-root user (uncomment and adjust).",
          "# User=aifight",
          "# Group=aifight",
          // Safe, non-breaking hardening for the root case: block privilege
          // escalation from the service and its children.
